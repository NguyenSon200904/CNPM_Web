    ...
    const fetchData = async () => {
      ...
      try {
        const responses = await Promise.allSettled([
          api.get("/api/inventory"), // Thêm /api/
          api.get("/api/receipts"), // Thêm /api/
          api.get("/api/export-receipts"), // Thêm /api/
          api.get("/api/accounts"), // Thêm /api/
          api.get("/api/suppliers"), // Thêm /api/
        ]);
        ...
        // Kiểm tra lỗi từng API
        if (inventoryResponse.status === "rejected") {
          console.error("Lỗi /api/inventory:", inventoryResponse.reason);
        }
        if (receiptsResponse.status === "rejected") {
          console.error("Lỗi /api/receipts:", receiptsResponse.reason);
        }
        if (exportReceiptsResponse.status === "rejected") {
          console.error(
            "Lỗi /api/export-receipts:",
            exportReceiptsResponse.reason
          );
        }
        if (accountsResponse.status === "rejected") {
          console.error("Lỗi /api/accounts:", accountsResponse.reason);
        }
        if (suppliersResponse.status === "rejected") {
          console.error("Lỗi /api/suppliers:", suppliersResponse.reason);
        }
        ...
        const inventoryData =
          inventoryResponse.status === "fulfilled"
            ...
            : [];
        const receiptsData =
          receiptsResponse.status === "fulfilled"
            ? receiptsResponse.value.data
            : [];
        const exportReceiptsData =
          exportReceiptsResponse.status === "fulfilled"
            ? exportReceiptsResponse.value.data
            : [];
        const accountsData =
          accountsResponse.status === "fulfilled"
            ...
            : [];
        const suppliersData =
          suppliersResponse.status === "fulfilled"
        ...
        console.log("Dữ liệu từ /api/receipts:", receiptsData);
        if (receiptsData && Array.isArray(receiptsData)) {
          ...
          console.warn("Dữ liệu từ /api/receipts không hợp lệ:", receiptsData);
        }

        console.log("Dữ liệu từ /api/export-receipts:", exportReceiptsData);
        if (exportReceiptsData && Array.isArray(exportReceiptsData)) {
          exportReceiptsData.forEach((exportReceipt) => {
            if (
          ...
          console.warn(
            "Dữ liệu từ /api/export-receipts không hợp lệ:",
            exportReceiptsData
          );
        }

        const productsData = Array.isArray(inventoryData)
        ...
        const invoicesData = [
          ...(Array.isArray(receiptsData)
            ...
            : []),
          ...(Array.isArray(exportReceiptsData)
            ? exportReceiptsData
        ...
        console.log("Dữ liệu từ /api/accounts:", accountsData);
        const accountsDataProcessed = Array.isArray(accountsData)
        ...
        setData({
          ...
          accounts: accountsDataProcessed,
        });

        setStats({
          productCount: Array.isArray(inventoryData)
            ? inventoryData.filter((item) => item.soLuongTonKho > 0).length
            : 0,
          supplierCount: Array.isArray(suppliersData)
            ? suppliersData.length
            : 0,
          accountCount: Array.isArray(accountsData) ? accountsData.length : 0,
        });
      } catch (error) {
